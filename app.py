...
def normalizar_referencia_base(ref):
    """
    BLINDAJE: Atrapa nulos reales, celdas en blanco o textos fantasma de Excel como "nan" o "None"
    """
...
def procesar_contpaq_core(file):
    ...
    # ---> DETECTOR DE FILAS FANTASMA (Ignorando la fila de TOTALES finales del reporte) <---
    ...
    is_total = raw_str[3].str.contains("TOTAL", case=False, na=False) | raw_str[2].str.contains("TOTAL", case=False, na=False)
    
    filas_fantasma = df[tiene_dinero & ~is_mov & ~is_saldo_ini & ~is_total].copy()
...
def analizar_saldos(movs, resumen):
    # Ahora mandamos TODOS los movimientos para que la matemática sea perfecta
...
if uploaded_file:
    with st.spinner("🚀 Extrayendo clientes y generando análisis..."):
        try:
            movs, resumen, filas_fantasma = procesar_contpaq_core(uploaded_file)
            df_audit = analizar_saldos(movs, resumen)
            
            # ---> AISLAR CULPABLES POTENCIALES <---
    ...
    col2.metric("Diferencia Matemática", f"${diferencia_total:,.2f}", delta_color="inverse" if abs(diferencia_total) > UMBRAL_TOLERANCIA else "normal")
    ...
    with st.expander("🤖 Ver Análisis Ejecutivo Automático", expanded=True):
        ...
        # 0. Filas Fantasma Reales (omitiendo la fila de Totales del reporte)
        if not filas_fantasma.empty:
            hubo_hallazgos = True
            st.error(f"👻 **ALERTA DE FILAS FANTASMA:** Encontramos **{len(filas_fantasma)} fila(s)** con dinero, pero que la herramienta se estaba saltando porque la fecha viene vacía o con un formato extraño en el reporte.")
        ...
        if not pagos_excedentes.empty:
            ...
            st.warning(f"⚠️ **{len(pagos_excedentes)} Facturas Pagadas de Más:** Se detectaron facturas donde el abono supera al cargo. El caso más fuerte es la Factura **{max_exc['referencia_norm']}** de **{max_exc['cliente']}**.")
        ...
        # 4. Alerta de Movimientos en Blanco (El blindaje que agregamos)
        if not movs_sin_referencia.empty:
            hubo_hallazgos = True
            total_fuga = movs_sin_referencia['saldo_neto'].sum()
            st.error(f"🔍 **DETECTOR DE FUGAS (MOVIMIENTOS SIN REFERENCIA):** Se detectaron **{len(movs_sin_referencia)} movimientos** donde quien capturó dejó la descripción/referencia en blanco. Estos movimientos suman un neto de **${abs(total_fuga):,.2f}** y causaban diferencias. La matemática ahora cuadra porque los hemos agrupado, pero debes revisar estas pólizas en tu sistema.")
            st.dataframe(movs_sin_referencia[["meta_codigo", "poliza", "fecha_raw", "concepto", "cargos", "abonos"]], use_container_width=True, hide_index=True)

        # 5. Descuadre General Real
        if abs(diferencia_total) > UMBRAL_TOLERANCIA:
            hubo_hallazgos = True
            st.error(f"❌ **DIFERENCIA MATEMÁTICA:** Hay una diferencia global de **${diferencia_total:,.2f}**. Revisa las cuentas marcadas en rojo en el Semáforo Contable.")
    ...
    with t4:
        # Gráficos ajustados para no fallar si todo cuadra a cero
        y_ok = saldo_total if abs(diferencia_total) <= UMBRAL_TOLERANCIA else (saldo_total - diferencia_total)
        fig = go.Figure(data=[
            go.Bar(name='Cuentas Cuadradas', x=['Matemática del Reporte'], y=[y_ok], marker_color='#2ecc71'),
